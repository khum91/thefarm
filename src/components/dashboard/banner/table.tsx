...
import { fetchFilteredBannerItem } from '@/data/actions/banner';
import Pagination from '@/components/common/pagination';
import BannerStatus from './status';
import Image from 'next/image';

export default async function bannerItemTable({
  query,
  currentPage,
}: {
  query: string;
  ...
  const response = await fetchFilteredBannerItem(query, currentPage);
              ...
              <div key={item.id} className="mb-2 w-full rounded-md bg-white p-4">
                <div className="flex items-center justify-between border-b pb-4">
                  <div className="mb-2 flex items-center">
                    ...
                    <p className='mr-5'>{item.link}</p>
                    <p className='mr-5'>{item.satus}</p>
                    <p className='mr-5'>{item.image}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            <tbody className="bg-white">
              {items?.map((item) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-3 py-3">
                    <BannerStatus status={item.status} />
                  </td>
                  <td className="whitespace-nowrap px-3 py-3">
                    <Image
                      ...
                      width={100}
                      height={100}
                      alt={`${item.name}'s banner picture`}
                    />
                  </td>
